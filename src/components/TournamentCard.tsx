...
const statusConfig = {
  ...
  in_progress: { label: 'En curso', color: 'text-[#7c3aed]', bg: 'bg-[#7c3aed]/10 border-[#7c3aed]/20', dot: 'bg-[#7c3aed]' },
...
export default function TournamentCard({ tournament }: { tournament: Tournament }) {
  ...
  return (
    <Link href={`/tournaments/${tournament.id}`}>
      <div className="bg-[#111] border border-[#222] rounded-xl p-5 hover:border-[#7c3aed]/50 hover:shadow-[0_0_24px_rgba(124,58,237,0.12)] transition-all duration-200 group cursor-pointer h-full flex flex-col">
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-white font-bold text-base group-hover:text-[#7c3aed] transition-colors line-clamp-2 leading-tight">
              {tournament.title}
            </h3>
          </div>
          {/* Prize */}
          <div className="ml-3 text-right flex-shrink-0">
            <div className="text-[#7c3aed] font-black text-xl leading-none">${tournament.prize_pool}</div>
