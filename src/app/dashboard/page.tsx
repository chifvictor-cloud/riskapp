  ...
  const stats = [
    {
      ...
      sub: 'MXN disponible',
      icon: <Wallet size={18} className="text-[#8b5cf6]" />,
      accent: true,
      link: '/wallet',
      linkLabel: 'Depositar',
    },
    ...
    },
    {
      ...
      icon: <Trophy size={18} className="text-[#8b5cf6]" />,
      iconBg: 'bg-[#8b5cf6]/10',
    },
    {
      label: 'Win Rate',
    ...
    <div className="min-h-screen bg-[#08071a]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
          <div>
            <p className="text-[#8b5cf6] text-sm font-semibold uppercase tracking-widest mb-1">Mi panel</p>
            <h1 className="text-3xl sm:text-4xl font-black text-white leading-none">
              Hola, <span style={{ color: '#8b5cf6' }}>{profile?.display_name || profile?.username}</span>
            </h1>
            <p className="text-[#888] text-sm mt-1.5">@{profile?.username}</p>
          </div>
          <Link
            href="/tournaments?create=1"
            className="inline-flex items-center gap-2 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-bold px-5 py-3 rounded-xl transition-colors shadow-[0_0_20px_rgba(139,92,246,0.2)]"
          >
            <Plus size={18} />
            Crear torneo
          </Link>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
          {stats.map((s, i) => (
            <div key={i} className={`bg-[#0f0e2a] border rounded-xl p-4 ${s.accent ? 'border-[#8b5cf6]/30' : 'border-[#201e50]'}`}>
              <div className="flex items-center justify-between mb-2">
                <span className="text-[#555] text-xs">{s.label}</span>
                <div className={`w-7 h-7 ${(s as any).iconBg ?? 'bg-[#8b5cf6]/10'} rounded-lg flex items-center justify-center`}>
                  {s.icon}
                </div>
              </div>
              <div className={`font-black text-xl ${s.accent ? 'text-[#8b5cf6]' : 'text-white'}`}>{s.value}</div>
              <div className="text-[#444] text-xs mt-0.5">{s.sub}</div>
              {s.link && (
                <Link href={s.link} className="inline-block mt-2 text-[#8b5cf6] text-xs font-semibold hover:underline">
                  {s.linkLabel} →
                </Link>
              )}
            </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-[#0d0c26] border border-[#1e1b4b] rounded-2xl p-5">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link href="/tournaments" className="text-[#8b5cf6] text-xs flex items-center gap-1 hover:underline">
              ...
              {recentMatches && recentMatches.length > 0 ? (
                <div className="space-y-2">
                  {recentMatches.map((m: any) => {
                    ...
                    return (
                      <div key={m.id} className="flex items-center gap-3 bg-[#0f0e2a] border border-[#1e1b4b] rounded-xl px-4 py-3">
                        ...
                        <div className="text-right flex-shrink-0">
                          {isPending ? (
                            <span className="text-[#8b5cf6] text-xs font-semibold">En curso</span>
                          ) : m.winner_id ? (
                            <>
                              ...
                              {isWinner && (
                                <p className="text-[#8b5cf6] text-xs">+${m.tournament?.prize_pool} MXN</p>
                              )}
                            </>
                          ) : (
                ...
                <div className="text-center py-10">
                  <Swords size={36} className="text-[#2d2960] mx-auto mb-3" />
                  <p className="text-[#888] text-sm">Aún no has jugado ninguna partida</p>
                  <Link
                    href="/tournaments"
                    className="inline-block mt-4 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-semibold px-5 py-2 rounded-lg text-sm transition-colors"
                  >
                    Buscar torneo
                  </Link>
                </div>
            ...
            <div className="bg-[#0d0c26] border border-[#1e1b4b] rounded-2xl p-5">
              ...
              {activeTournaments && activeTournaments.length > 0 ? (
                  ...
                  {activeTournaments.map((entry: any) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-center justify-between bg-[#0f0e2a] border border-[#1e1b4b] hover:border-[#8b5cf6]/30 rounded-xl px-4 py-3 transition-colors group"
                      >
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                          <span className={`w-2 h-2 rounded-full ${t.status === 'open' ? 'bg-green-400 animate-pulse' : 'bg-[#8b5cf6]'}`} />
                ...
                <div className="text-center py-8">
                  <Trophy size={28} className="text-[#2d2960] mx-auto mb-2" />
                  <p className="text-[#888] text-xs">Sin torneos activos</p>
                  <Link
                    href="/tournaments"
                    className="inline-block mt-3 text-[#8b5cf6] text-xs hover:underline font-semibold"
                  >
                    Ver disponibles
                  </Link>
                </div>
            ...
            <div className="bg-[#0d0c26] border border-[#1e1b4b] rounded-2xl p-5">
              ...
              <div className="space-y-2.5 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium">
                    {profile?.fortnite_username
                      ? profile.fortnite_username
                      : <Link href="/profile" className="text-[#8b5cf6] hover:underline text-xs">Agregar usuario</Link>
                    }
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-[#8b5cf6] font-bold">{winRate}%</span>
                </div>
              </div>

              <Link
                href="/profile"
                className="mt-4 w-full block text-center text-sm bg-[#0f0e2a] hover:bg-[#1e1b4b] border border-[#272454] text-white font-medium py-2.5 rounded-xl transition-colors"
              >
                Editar perfil
              </Link>
            </div>
