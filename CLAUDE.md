...
- **Auditoría — v20 (17-jul):** Eliminada `refund_withdrawal` (v8): SECURITY DEFINER huérfana, ejecutable por `authenticated`, permitía auto-acreditarse balance retirable como dinero real. REVOKE + DROP. Verificada en DB (0 filas). Detectada durante auditoría de solo lectura del flujo de dinero de torneos; el rollback legítimo de retiros lo cubre `admin_reject_withdrawal` (v9).
- **v21 — Rake de torneos (17-jul):** comisión configurable vía `app_config` (`tournament_rake_pct`, default 0.10). Se cobra al pagar el premio en `submit_match_result` y `admin_resolve_dispute`; ganador recibe pot*(1-rake), plataforma retiene pot*rake registrado como transaction `type='rake'`. Helper `get_tournament_rake_pct()` SECURITY INVOKER (corre bajo el definer padre). Invariante payout+rake=pot, centavo del trunc va al ganador. Verificado en vivo: torneo $50 c/u → $90 al ganador + $10 rake. Primer ingreso real de la plataforma. DEUDA: transaction rake queda a nombre del ganador (`user_id` NOT NULL) — mover a cuenta plataforma/ledger cuando haya ledger auditable (R2).
